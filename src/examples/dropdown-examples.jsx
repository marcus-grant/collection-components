...
const numSequences = [ // { title: 'Triangle', nums: [1, 3, 6, 10, 15, 21, 28] }, { title: 'Fibonacci', nums: [1, 1, 2, 3, 5, 8, 13] }, { title: 'Prime', nums: [1, 2, 3, 5, 7, 11, 13] },
    ...
    return (
      <div className="demo">
        ...
        <Dropdown
          ...
          itemRenderer={(_z, index) => (
            <ListCell
              key={`lc${index}`} // eslint-disable-line
              cellKey={`c[${index}]`}
              cellIndex={index}
              rightAccessory={<StatusButton key={index} status={btnStatuses[index]} />}
            />
          )}
        />
      </div>
